...
class SeqAction(AnimationGroup):
    ...
    .. manim:: SeqActionDocExample
       ...
       class SeqActionDocExample(MovingCameraScene):
           def construct(self):
               ...
               for anime in SeqAction.introduce_actors(
                   actor_client, actor_server, actor_delivery, actor_db
               ):
                   self.play(anime)
               ...
               # existing animations
               for db_anime, server_anime in zip(
                   delivery_to_db_animes, delivery_to_server_animes
               ):
                   self.play(db_anime)
    ...
    def subject_gives_gift_to_target(
        ...
        gift.move_to(
            act_start.get_center(), aligned_edge=(RIGHT if is_move_left else LEFT)
        )
        ...
        yield Succession(
            ...
            AnimationGroup(
                FadeIn(iobj_planned_path, run_time=0.2), FadeIn(post_move_gift_label)
            ),
        )